...
def initialize_vault(
    ...
    if user_record:
        ...
        existing_vaults = db.get_user_vaults(username)
    ...
    vault_id = db.create_vault(user_id, vault_name)
    vault_policy = db.create_vault_policy(vault_id, hardware_gate_required, software_only_enabled) 
    ...
    db.save_auth_credentials(vault_id, passphrase_salt_b64, wrapped_master_key, wrapped_nonce, kdf_params)
    ...
    if keypad_pin:
        ...
        db.save_hardware_auth(vault_id, hashed_pin, pin_salt_b64)
    ...
    db.save_vault_data(vault_id, encrypted_vault_blob, nonce)
    db.write_access_log(vault_id, user_id, "initialize", "hardware_gated", True, "Vault initialized")
...
def verify_hardware_pin(vault_id: int, pin_attempt: str) -> bool:
    ...
    """
    hardware_auth = db.load_hardware_auth(vault_id)
    ...
    if crypto.verify_keypad_pin(pin_attempt, stored_hash, salt):
        ...
        if db.get_failed_pin_attempts(vault_id) >= 3:
...
def open_passphrase_window(vault_id: int) -> None:
    """
        ...
        Returns: None
    """
    vault_policy = db.load_vault_policy(vault_id)
    if not vault_policy:
...
def unlock_with_passphrase(
    ...
    auth_credentials = db.load_auth_credentials(vault_id)
    ...
    try:
        ...
        db.write_access_log(vault_id, None, "unlock", auth_method, False, "Invalid passphrase")
        raise ValueError("Invalid passphrase")
        
    vault_data = vault.load_decrypted_vault(vault_id, master_key)
    vault_record = db.load_vault(vault_id)
    ...
    db.write_access_log(vault_id, user_id, "unlock", auth_method, True, "Vault unlocked")
...
def unlock_software_only(vault_id: int, passphrase: str) -> dict:
    ...
    try:
        ...
        db.write_access_log(vault_id, None, "unlock", "software_only", False, "Invalid passphrase")
        raise ValueError("Invalid passphrase")
    ...
    db.write_access_log(vault_id, user_id, "unlock", "software_only", True, "Vault unlocked")
